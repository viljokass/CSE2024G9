...
def test_order_matching(client):
    ...
    client.post("/v1/orders", json={"type": "bid", "price": 210.0, "quantity": 500})

    response = client.post(
        "/v1/orders", json={"type": "offer", "price": 225.0, "quantity": 750}
    )

    assert response.status_code == 406

    client.post("/v1/orders", json={"type": "offer", "price": 205.0, "quantity": 500})

    response = client.get("/v1/trades")
    assert response.status_code == 200
    assert b'"quantity": 500, "price": 210.0' in response.data

    client.post("/v1/orders", json={"type": "offer", "price": 200.0, "quantity": 1500})
    ...
    client.post("/v1/orders", json={"type": "offer", "price": 200.0, "quantity": 750})
